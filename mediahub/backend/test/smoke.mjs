...
const port=18080; const base=`http://127.0.0.1:${port}`;
...
try {
  ...
  const created=await fetch(`${base}/v1/device-pairing`,{method:"POST"}).then(r=>r.json()); assert.ok(created.pairingId && created.deviceSecret && created.userCode);
  ...
  const session=await fetch(`${base}/v1/device-pairing/${created.pairingId}`,{headers:{"x-mediahub-device-secret":created.deviceSecret}}).then(r=>r.json()); assert.equal(session.status,"approved"); assert.ok(session.session.accessToken && session.session.refreshToken);
